...
function FilmLibrary() {
    ...
    const db = new sqlite.Database('lab2022-03-17/ex2/films.db', (err) => {
        if (err) throw(err);
    })
    ...
    this.dbAddFilm = (f) => {
        return new Promise((resolve, reject) => {
            ...
            db.run(sql, [f.id, f.title, f.fav, dayjs(f.date).format('YYYY-MM-DD'), f.rating], (err) => {
                if (err) reject("Error inserting the Film");
                else resolve("Film inserted succesfully");
            });
        });
    }

    this.dbDeleteFilm = (id) => {
        return new Promise((resolve, reject) => {
            const sql = 'DELETE FROM films WHERE id = ?';
            db.run(sql, [id], (err) => {
                if (err) reject("Error deleting the Film");
                else resolve("Film deleted succesfully");
            });
        });
    }

    this.dbDeleteAllDates = () => {
        return new Promise((resolve, reject) => {
            const sql = 'UPDATE films SET watchdate = NULL';
            db.run(sql, (err) => {
                if (err) reject("Error deleting dates");
                else resolve("Dates deleted succesfully");
            });
        });
    }
...
async function main() {
    ...
    const addNew = await fl.dbAddFilm(new Film(7, "Inception", 1, new Date(), 5)).catch(console.log);
    console.log(addNew);
    const deleteFilm = await fl.dbDeleteFilm(2).catch(console.log);
    console.log(deleteFilm);
    const deldates = await fl.dbDeleteAllDates().catch(console.log);
